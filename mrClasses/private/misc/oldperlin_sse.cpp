...
//	q = g2[ b00 ];		$@$SC$@$ get random gradient $@$EC$@$      
//	u = rx0 * q[0] + ry0 * q[1];
...
//    a = u + sx * (v-u);  $@$SC$@$ get value at distance sx between u & v $@$EC$@$
	tmp  = _mm_sub_ps (v,u);
...
//	result = 1.5 * (a + sy * (b-a))	$@$SC$@$ interpolate in y $@$EC$@$   why 1.5 ??
	tmp  = _mm_sub_ps(b,a);
...
//	q = g3[ b00 ];		$@$SC$@$ get random gradient $@$EC$@$      
...
//	result = 1.5 * (c + sz * (d-c))	$@$SC$@$ interpolate in y $@$EC$@$   why 1.5 ??
	tmp  = _mm_sub_ps(d,c);
